...
```
인터넷 ── HTTPS ──▶ DSM 리버스 프록시(8443) ──▶ 백엔드 컨테이너(호스트 18080→8080) ──▶ MariaDB(내부 3307)
```
...
기동 후 LAN에서 먼저 확인: `http://192.168.35.2:18080/api/items`
(호스트 포트는 다른 컨테이너와 충돌하지 않도록 `18080`으로 뒀다. 컨테이너 내부는 8080.)
